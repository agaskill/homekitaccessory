...
{
    public class TestSRPExchange
    {
        private const string N1024 = @"";
        private static byte[] g1024 = { 2 };

        [Fact]
        public void TestHAP()
        {
            byte[] N = Utilities.ParseByteArray(
          ...
          E0FD108E 4B82D120 A93AD2CA FFFFFFFF FFFFFFFF");
            ...
            var clientSecret = @"60975527 035CF2AD 1989806F 0407210B C81EDC04 E2762A56 AFD529DD DA2D4393".ParseByteArray();
            var serverSecret = @"E487CB59 D31AC550 471E81F0 0F6928E0 1DDA08E9 74A004F4 9E61F5D1 05284D20".ParseByteArray();
            var expectedClientPublic = @"FAB6F5D2 615D1E32 3512E799 1CC37443 F487DA60 4CA8C923 0FCB04E5 41DCE628
  ...
  81E23F27 A231D184 32D7D1BB 55C28AE2 1FFCF005 F57528D1 5A88881B B3BBB7FE".ParseByteArray();
            var expectedServerPublic = @"40F57088 A482D4C7 733384FE 0D301FDD CA9080AD 7D4F6FDF 09A01006 C3CB6D56
  ...
  89CCA306 CC370A0B 18C8B886 E95DA0AF 5235FEF4 393020D2 B7F30569 04759042".ParseByteArray();

            var salt = "BEB25379 D1A8581E B5A72767 3A2441EE".ParseByteArray();

            var expectedVerifier = Utilities.ParseByteArray(
  ...
  C7EB714F 6FF7D44C D5B86F6B D1158109 30637C01 D0F6013B C9740FA2 C633BA89");
            ...
            var expectedPremaster = Utilities.ParseByteArray(
  ...
  E7287234 58538858 D715B7B2 47406222 C1019F53 603F0169 52D49710 0858824C");
            ...
            var userVerifier = UserVerifier.Create(
                username, password, groupParameters, salt, hash);

            Assert.Equal(expectedVerifier, userVerifier.Verifier);

            var server = new SRPServer(new TestUserStore(userVerifier), hash);
            var client = new SRPClient(hash);

            var srpServer = server.ClientHello(username, serverSecret);

            Assert.Equal(expectedServerPublic, srpServer.ServerPublic);

            var clientSession = client.ServerExchange(username, password, srpServer, clientSecret);

            Assert.Equal(expectedClientPublic, clientSession.ClientPublic);

            Assert.Equal(expectedPremaster, clientSession.PremasterSecret);

            var serverPremaster = server.ClientExchange(clientSession.ClientPublic);
            Assert.Equal(expectedPremaster, serverPremaster);

            var expectedKey = Utilities.ParseByteArray(
  ...
  6AF245C2 093F98C3 651BCA83 AB8CAB2B 580BBF02 184FEFDF 26142F73 DF95AC50");
            ...
            var sessionKey = hash.ComputeHash(serverPremaster);
            Assert.Equal(expectedKey, sessionKey);
        }
        ...
        public void Test1()
        {
            var username = "alice";
            var password = "password123";
            var salt = Utilities.ParseByteArray(@"BEB25379D1A8581EB5A727673A2441EE");
            var hash = SHA1.Create();
            byte[] N = Utilities.ParseByteArray(N1024);
            byte[] g = g1024;

            var groupParameters = SRPGroupParameters.Rfc5054_1024;

            var expectedVerifier = Utilities.ParseByteArray(@"");
            var verifier = UserVerifier.Create(username, password, SRPGroupParameters.Rfc5054_1024, salt, hash);

            Assert.Equal(N, verifier.Prime);
            Assert.Equal(g, verifier.Generator);
            Assert.Equal(salt, verifier.Salt);
            Assert.Equal(expectedVerifier, verifier.Verifier);
            var userStore = new TestUserStore(verifier);
            var server = new SRPServer(userStore, SHA1.Create());
            var client = new SRPClient(SHA1.Create());

            var serverSRP = server.ClientHello(username);

            //var expectedServerPublic = Utilities.ParseByteArray(
            ...
            //Assert.Equal(expectedServerPublic, serverSRP.ServerPublic);

            var clientSession = client.ServerExchange(username, password, serverSRP);

            //var expectedClientPublic = Utilities.ParseByteArray(
            ...
            //var expectedPremaster = Utilities.ParseByteArray(@"");
            ...
            Assert.Equal(clientSession.PremasterSecret, serverPremaster);
        }
    }
}
